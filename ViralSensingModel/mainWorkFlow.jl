...
varNames = ["IFN","IFNe","STATP","IRF7","IRF7P","Infected","Virus"]
...
function Model!(dy,y,par,t)
  ...
  dy[1]=k11*v+(k12*(v^n))/(k13+(v^n))+k14*y[5]-k21*y[1] #IFN in cells
  dy[2]=k21*y[1]-y[2]*tau2 #IFN in environment
  dy[3]=(k31*y[2])/(k32+k33*y[2])-y[3]*tau3 #STATP
...
u0 = [7.94, 0.01, 12.2, 14.15, 0.01, 1, 7.5E-2] #Initial Conditions
...
priors[5]=Uniform(0,1) #tau2, IFNe degradation
...
newProb = ODEProblem(Model!,u0,[0.25,18],bestPar)
